...
import net.minecraft.src.Entity404;
...
import net.minecraft.src.HorrorState;
...
import net.minecraft.src.ScreamerManager;
...
public abstract class Minecraft implements Runnable {
	...
	public void startGame() throws LWJGLException {
		...
		Display.setTitle("Minecraft 1.1.1 Free");
	...
	public void displayGuiScreen(GuiScreen var1) {
		if(!(this.currentScreen instanceof GuiUnused)) {
			...
			} else if(var1 == null && this.thePlayer.getEntityHealth() <= 0) {
	...
	public void changeWorld(World var1, String var2, EntityPlayer var3) {
		...
		if(var1 != null) {
			...
			// Horror mod: Initialize state and spawn Entity404
			if(!var1.multiplayerWorld) {
				HorrorState.reset();

				// Spawn Entity404 stalker
				if(this.thePlayer != null) {
					Entity404 stalker = new Entity404(var1, this.thePlayer);
					double angle = var1.rand.nextDouble() * Math.PI * 2;
					double distance = 50.0D + var1.rand.nextDouble() * 20.0D;
					double spawnX = this.thePlayer.posX + Math.cos(angle) * distance;
					double spawnZ = this.thePlayer.posZ + Math.sin(angle) * distance;
					double spawnY = var1.getHeightValue((int)spawnX, (int)spawnZ) + 1;
					stalker.setPosition(spawnX, spawnY, spawnZ);
					var1.spawnEntityInWorld(stalker);
