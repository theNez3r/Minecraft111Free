...
public class GuiIngameMenu extends GuiScreen {
	...
	public void initGui() {
		...
		// Horror mod: Check if exit trap should be active
		boolean exitTrapActive = HorrorState.shouldActivateExitTrap();

		GuiButton saveQuitButton = new GuiButton(1, this.width / 2 - 100, this.height / 4 + 120 + var1, StatCollector.translateToLocal("menu.returnToMenu"));
		if(this.mc.isMultiplayerWorld()) {
			saveQuitButton.displayString = StatCollector.translateToLocal("menu.disconnect");
		}

		// Disable button if exit trap is active
		if(exitTrapActive) {
			saveQuitButton.enabled = false;
			HorrorState.exitTrapActive = true;
		}

		this.controlList.add(saveQuitButton);
	...
	protected void actionPerformed(GuiButton var1) {
		...
		if(var1.id == 1) {
			// Horror mod: Show message if exit trap is active
			if(HorrorState.exitTrapActive) {
				if(this.mc.thePlayer != null) {
					this.mc.thePlayer.addChatMessage("\u00a7cYou can't leave yet...");
				}
				return;
			}
