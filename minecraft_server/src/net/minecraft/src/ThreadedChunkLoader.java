...
public class ThreadedChunkLoader implements IChunkLoader, IThreadedFileIO {
		...
		synchronized(var6) {
			if(this.pendingChunkCoords.contains(var5)) {
				for(int var7 = 0; var7 < this.pendingChunkList.size(); ++var7) {
					if(((ThreadedChunkLoaderPending)this.pendingChunkList.get(var7)).field_40613_a.equals(var5)) {
						var4 = ((ThreadedChunkLoaderPending)this.pendingChunkList.get(var7)).field_40612_b;
						break;
					}
				}
			}
		...
		synchronized(var3) {
			if(this.pendingChunkCoords.contains(var1)) {
				for(int var4 = 0; var4 < this.pendingChunkList.size(); ++var4) {
					if(((ThreadedChunkLoaderPending)this.pendingChunkList.get(var4)).field_40613_a.equals(var1)) {
	...
	public boolean writeNextIO() {
		...
		synchronized(var2) {
			...
			this.pendingChunkCoords.remove(var1.field_40613_a);
		}
	...
	public void writeChunk(ThreadedChunkLoaderPending var1) throws IOException {
		DataOutputStream var2 = RegionFileCache.getChunkOutputStream(this.chunkSaveLocation, var1.field_40613_a.chunkXPos, var1.field_40613_a.chunkZPos);
		CompressedStreamTools.writeTo(var1.field_40612_b, var2);
		var2.close();
	}
